...
появился список questions_list (строка 31)
этот список заполняется нужными значениями прямо в коде (строки 33-36)
Возможность прохождения по списку обеспечивает функция next_question()
(она использует свойство window.cur_question - индекс текущего вопроса в списке)
Нажатие на кнопку теперь может вызывать не только check_answer, но и next_question
Разбираться, что конкретно делать, поручено функции click_OK()
"""
